...
import os
import time
from functionfiles import draw_boxes 
...
class ASOne:
    ...
    def track_video(self,
        ...
        for (bbox_details, frame_details, vegetables) in self._start_tracking(video_path, config):
            # yeild bbox_details, frame_details to main script
            yield bbox_details, frame_details, vegetables
    ...
    def track_webcam(self,
        ...
        for (bbox_details, frame_details, vegetables) in self._start_tracking(cam_id, config):
            # yeild bbox_details, frame_details to main script
            yield bbox_details, frame_details, vegetables
    ...
    def _start_tracking(self,
        ...
        while True:
            ...
            im0, object_counter = draw_boxes(im0,
                                   bboxes_xyxy,
                                   class_ids,
                                   identities=ids,
                                   draw_trails=draw_trails,
                                   class_names=class_names)
            #print(object_counter.keys())
            currTime = time.time()
            ...
            yield (bboxes_xyxy, ids, scores, class_ids), (im0 if display else frame, frame_id-1, fps), (object_counter)#.keys())
        #print(object_counter.keys())
        tac = time.time()
